...
from utils import create_outpath, forward_pass, recovery_intensity, plot_hawkes_recovery_intensity

signal.signal(signal.SIGINT, lambda sig, frame: sys.exit(0))

parser = argparse.ArgumentParser('hawkes')
parser.add_argument('--niters', type=int, default=300)
parser.add_argument('--dataset', type=str, default='hawkes')
...
if __name__ == '__main__':
    ...
    with open('../data/hawkes/train.pkl', 'rb') as file:
        train_time = pickle.load(file)
        
    with open('../data/hawkes/test.pkl', 'rb') as file:
    ...
    it0 = 0
    optimizer = optim.Adam([{'params': sde.parameters()},
                            {'params': eta0, 'lr': 1.0e-1},
    ...
    while it < args.niters:
        ...
        if it in check_list:
            ...
            t = t.view(-1)
            lmbda = lmbda.view(-1)
            plot_hawkes_recovery_intensity(t, lmbda, seq=test_time[4], xlabel='$t$', ylabel='$\lambda_t$', title='Hawkes')
            plt.savefig(f"hawkes_{it}.png")
