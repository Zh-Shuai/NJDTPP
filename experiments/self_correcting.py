...
from utils import create_outpath, forward_pass, recovery_intensity, plot_self_correcting_recovery_intensity


signal.signal(signal.SIGINT, lambda sig, frame: sys.exit(0))

parser = argparse.ArgumentParser('self_correcting')
parser.add_argument('--niters', type=int, default=300)
parser.add_argument('--dataset', type=str, default='self_correcting')
...
outpath = create_outpath('self_correcting')
...
if __name__ == '__main__':
    ...
    with open('../data/self_correcting/train.pkl', 'rb') as file:
        train_time = pickle.load(file)
        
    with open('../data/self_correcting/test.pkl', 'rb') as file:
        test_time = pickle.load(file)

    train_time = [torch.tensor(train_time[i], device=device, dtype=torch.float32) for i in range(len(train_time))]
    ...
    test_time = [torch.tensor(test_time[i], device=device, dtype=torch.float32) for i in range(len(test_time))]

    # initialize / load model
    dim_eta = 1
    sde = NJDSDE(device, dim_eta, dim_hidden=32, num_hidden=2, batch_size=args.batch_size, sigma=0.1, activation=nn.Tanh()).to(device)
    ...
    while it < args.niters:
        ...
        if it in check_list:
            ...
            t = t.view(-1)
            lmbda = lmbda.view(-1)
            plot_self_correcting_recovery_intensity(t, lmbda, seq=test_time[4], xlabel='$t$', ylabel='$\lambda_t$', title='Self_Correcting')
            plt.savefig(f"self_correcting_{it}.png")
