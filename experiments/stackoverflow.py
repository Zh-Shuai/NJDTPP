...
parser = argparse.ArgumentParser('stackoverflow')
...
outpath = create_outpath('stackoverflow')
...
if __name__ == '__main__':
    ...
    train_time, train_type, train_num_types, train_seq_lengths, train_mask = \
        process_loaded_sequences(device=device, source_dir='../data/stackoverflow/train.pkl', split='train')
    dev_time, dev_type, dev_num_types, dev_seq_lengths, dev_mask = \
        process_loaded_sequences(device=device, source_dir='../data/stackoverflow/dev.pkl', split='dev')
    test_time, test_type, test_num_types, test_seqs_lengths, test_mask = \
        process_loaded_sequences(device=device, source_dir='../data/stackoverflow/test.pkl', split='test')
    ...
    sde = NJDSDE(device, dim_eta=dim_eta, dim_hidden=16, num_hidden=2, batch_size=args.batch_size, sigma=0.01, activation=nn.Tanh()).to(device)
    eta0 = torch.empty(dim_eta, device=device).normal_(mean=0, std=0.1).requires_grad_()
    it0 = 0
    optimizer = optim.Adam([{'params': sde.parameters()},
                            {'params': eta0, 'lr': 1.0e-1, 'weight_decay': 1e-5},
