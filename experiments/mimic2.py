...
if __name__ == '__main__':
    ...
    train_time, train_type, train_num_types, train_seq_lengths, train_mask = \
        process_loaded_sequences(device=device, source_dir='../data/mimic2/train.pkl', split='train')
    dev_time, dev_type, dev_num_types, dev_seq_lengths, dev_mask = \
        process_loaded_sequences(device=device, source_dir='../data/mimic2/dev.pkl', split='dev')
    test_time, test_type, test_num_types, test_seqs_lengths, test_mask = \
        process_loaded_sequences(device=device, source_dir='../data/mimic2/test.pkl', split='test')
    ...
    optimizer = optim.Adam([{'params': sde.parameters(), 'lr': 1e-3, 'weight_decay': 1e-5},
                            {'params': eta0, 'lr': 1e-1, 'weight_decay': 1e-5},
                            ])
